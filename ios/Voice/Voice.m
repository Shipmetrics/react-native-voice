...
- (void) setupAndStartRecognizing:(NSString*)localeStr {
    ...
    self.recognitionRequest.taskHint = SFSpeechRecognitionTaskHintSearch;
    ...
    self.recognitionTask = [self.speechRecognizer recognitionTaskWithRequest:self.recognitionRequest resultHandler:^(SFSpeechRecognitionResult * _Nullable result, NSError * _Nullable error) {
        ...
        BOOL handleAsFinal = isFinal || self.recognitionTask.isCancelled || self.recognitionTask.isFinishing;
        
        [self sendResult:nil:result.bestTranscription.formattedString :transcriptionDics :handleAsFinal];
...
- (void) sendResult:(NSDictionary*)error :(NSString*)bestTranscription :(NSArray*)transcriptions :(BOOL)isFinal {
    if (error != nil) {
        [self sendEventWithName:@"onSpeechError" body:@{@"error": error}];
    }
    if (bestTranscription != nil && isFinal == YES) {
        [self sendEventWithName:@"onSpeechResults" body:@{@"value":@[bestTranscription]} ];
    } else if (isFinal == YES) {
        [self sendEventWithName:@"onSpeechResults" body:@{@"value":transcriptions} ];
    }
    if (transcriptions != nil) {
        [self sendEventWithName:@"onSpeechPartialResults" body:@{@"value":transcriptions} ];
    }
    if (isFinal == YES) {
        NSNumber* isFinalNum = @(isFinal);
        [self sendEventWithName:@"onSpeechRecognized" body: @{@"isFinal": isFinalNum}];
    }
}
